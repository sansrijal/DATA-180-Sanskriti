...
```{r}
clusture <- hclust(dist(Mammals_Milk_Std), method = "complete")
plot(clusture, main = "Mammals Clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage",sub = "", xlab = "Mammals", cex.lab = 1.25, hang = -1, frame.plot = TRUE, labels = FALSE)
```
...
table_km_3 <- arrange(Mammals_Milk_Clustered, km_3)
table_km_4 <- arrange(Mammals_Milk_Clustered, km_4)
...
```{r}
MMilk_Agg_K3 <- Mammals_Milk_Clustered %>%
  group_by(km_3) %>%
  summarize( Count = n(), Mean_Water = round(mean(Water),2), Mean_Protein = round(mean(Protein),2), Mean_Fat=round(mean(Fat),2), Mean_Lactose = round(mean(Lactose),2), Mean_Ash = round(mean(Ash),2))

MMilk_Agg_K4 <- Mammals_Milk_Clustered %>%
  group_by(km_3) %>%
  summarize( Count = n(), Mean_Water = round(mean(Water),2), Mean_Protein = round(mean(Protein),2), Mean_Fat=round(mean(Fat),2), Mean_Lactose = round(mean(Lactose),2), Mean_Ash = round(mean(Ash),2))
...
graph1 <- ggplot(Mammals_Milk_Clustered, aes(x = factor(km_3), y = Water)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Water", title = "Water Content")

graph2 <- ggplot(Mammals_Milk_Clustered, aes(x = factor(km_3), y = Protein)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Protein", title = "Protein Content")

graph3 <- ggplot(Mammals_Milk_Clustered, aes(x = factor(km_3), y = Fat)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Fat", title = "Fat Content")

graph4 <- ggplot(Mammals_Milk_Clustered, aes(x = factor(km_3), y = Lactose)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Lactose", title = "Lactose Content")

graph5 <- ggplot(Mammals_Milk_Clustered, aes(x = factor(km_3), y = Ash)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Ash", title = "Ash Content")
...
for (k in 1:8) {
  ...
  wgss[k] <- sum(km$withinss)
}
plot(1:8, wgss, type = "b", pch = 19, frame.plot = FALSE,
     main = "WGSS Number of Clusters",
     xlab = "Number of Clusters", ylab = "WGSS")
elbow <- which(diff(wgss) == max(diff(wgss))) + 1
abline(v = elbow, col = "red", lty = 2)
text(elbow, max(wgss), labels = paste("K =", elbow), pos = 3, col = "red")
```

The elbow plot suggests 8 clusters based on the elbow method. 
It is similar to the dendogram. (-2)
