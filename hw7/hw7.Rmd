...
#library(tidyverse)
library(tm)
news<-read.csv("/Users/sanskriti/Desktop/DATA-180-Sanskriti/hw7/news.csv",header=T)
```

Read also the positive and negative word dictionaries, both found on Github. This will later come in handy:

```{r}
posWords <- scan("/Users/sanskriti/Desktop/DATA-180-Sanskriti/hw7/positive-words.txt", character(0), sep = "\n")  # 2006 items
negWords <- scan("/Users/sanskriti/Desktop/DATA-180-Sanskriti/hw7/negative-words.txt", character(0), sep = "\n")  # 4783 items
head(posWords,15)
head(negWords,15)
```
...
wordCorpus <- tm_map(wordCorpus, content_transformer(customFunct))
...
Term document matrix is a mathematical matrix that represents the frequency of words or terms. 
```{r}
...
```{r}
topWordCount <- wordCounts[wordCounts >= 50]
sorted <- sort(topWordCount, decreasing = TRUE)
barplot(sorted, col= "pink", main = "Words appearing 50 Times",
xlab = "word", ylab = "count", las = 2, cex.names = 0.75)
...
posMatch <- match(names(wordCounts), posWords, nomatch = 0)
posMatch <- posMatch != 0
posMatch <- wordCounts[posMatch]
barplot(posMatch[posMatch >20], las = 2, cex.names = 0.75)
posPercentage <- sum(posMatch) / sumWords
print(posPercentage*100)

negMatch <- match(names(wordCounts), negWords, nomatch = 0)
negMatch <- negMatch != 0
negMatch <- wordCounts[negMatch]
 barplot(negMatch[negMatch>20],las=2,cex.names=0.75)
 print((sum(negMatch)/sumWords)*100)
...
```{r}
library(ggplot2)
ggplot(news, aes(x=factor(yearmonth, level=unique(yearmonth)), y = count)) + 
  geom_bar(stat = 'identity', fill = "pink") + 
  labs(title = "Frequency of Articles by Year and Month", x = "year/month", y = "Num of Articles") + theme(axis.text=element_text(size=4,angle=90))
```
...
```{r}
words <-termFreq(charVector, control = list(removePunctuation=TRUE, stopwords=TRUE))
sort(words, decreasing = TRUE)[1:20]
```
...
```{r}
library(ngram)
 Twowords <-paste(unlist(charVector), collapse=" ")
 pairs <- ngram(Twowords, n=2)
 head(get.phrasetable(pairs),20)
```
...
news_dtm <- dfm(newscorpus,
 tolower = TRUE, remove_punct = TRUE, remove_symbols = TRUE, remove_numbers = TRUE, remove = stopwords("english"), stem = TRUE)
news <- dfm_remove(news_dtm,c(':', '=', 'nt','mp'))
news <- dfm_trim(news_dtm, min_termfreq=50)
head(news_dtm)
```
...
```
I was surprised to see words like powertwo, Darwin, Tasmania and not surprised by works liked jail,parliament, violence.
...
```{r}
topMod <- convert(news_dtm, to = 'topicmodels')
topic_model <- LDA(topMod, method = "VEM", control = list(seed=1234), k=8)
terms(topic_model, 8)
```
...
topWords %>%
  ...
  ggplot(aes(beta, term, fill = factor(topic))) + geom_bar(stat = "identity") +
  geom_col(show.legend = FALSE) +
  labs(title = "Top 10 Words with Highest Beta for Each Topic", x = "Beta Value", y = "Word") +
  facet_wrap(~topic, scales = "free") + 
  scale_y_reordered()
  
```
Topic one has the highest beta for the word 'man' with a value of almost 0.008. Other top words inclue 'nsw' with a value greater than 0.004 and fire with a value of 0.004. This topic seems to be about a fire in New South Wales. 
For topic three the top words with their beta values respectively are 'man' with greater than 0.006, 'murder' with 0.006, 'face' and 'Australian'with a value slightly less than 0.006. This topic could be related to muders in Australia.
# Question 16
...
```{r}
gammaVal <- tidy(topic_model, matrix = "gamma")
documents <- gammaVal %>%
  group_by(topic) %>%
  slice_max(gamma, n = 5) %>%
  ungroup() %>%
  arrange(topic, -gamma)
ggplot(documents, aes(x = reorder(document, -gamma), y = gamma, fill = factor(topic)))  + geom_bar(stat = "identity") + labs(title = "Top 5 Documents with Highest Gamma for Each Topic",
       x = "Document", y = "Gamma Value") +
...
```
Yes because each word under each topic are related or similar to one another and match headlines. It would make sense to assign them together under a topic.
